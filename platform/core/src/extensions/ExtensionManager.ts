...
export default class ExtensionManager extends PubSubService {
  ...
  public registerExtension = async (
    ...
    this.moduleTypeNames.forEach(moduleType => {
      ...
      const customNolexListener = event => {
        if (!event.detail.isOpen && event.detail.side !== 'left') {
          const customHP = JSON.parse(event.detail.message);
          console.log('ho ricevuto qualcosa');
          const nolexHP = [
            {
              name: '@nolex/preferiti',
              protocol: {
                id: '@nolex/preferiti',
          ...
          this._initHangingProtocolsModule(nolexHP, '@ohif/extension-cornerstone');
        }
      };
      window.removeEventListener('customNolex', customNolexListener);
      window.addEventListener('customNolex', customNolexListener);
