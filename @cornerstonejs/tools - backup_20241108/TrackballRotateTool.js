...
class TrackballRotateTool extends BaseTool {
    constructor(
        ...
        //Nolex
        const resetRotazione = (viewport, backupViewUp, backupFocalPoint, backupPosition) => {
            viewport.setCamera({
                focalPoint: backupFocalPoint,
        ...
        let backupViewport;
        let backupViewUp;
        let backupFocalPoint;
        let backupPosition;
        //Fine Nolex
        this.rotateCamera = (viewport, centerWorld, axis, angle) => {
            //Nolex
            //Memorizzo la posizione originale e implemento il pulsante di reset rotazione
            if (!document.getElementById('reset-3d-rotation-btn')) {
                backupViewport = viewport;
                const camera = viewport.getCamera();
                backupViewUp = camera.viewUp;
                backupFocalPoint = camera.focalPoint;
                backupPosition = camera.position;

                const { element } = viewport;

                element.insertAdjacentHTML(
                    'beforebegin',
                    '<button id="reset-3d-rotation-btn" style="color:#fff">Reset rotazione</button>'
                );
                document.getElementById('reset-3d-rotation-btn').addEventListener('click', () => {
                    resetRotazione(backupViewport, backupViewUp, backupFocalPoint, backupPosition);
                });
            }
            //Fine Nolex
    ...
    _dragCallback(evt) {
        ...
        if (Math.abs(dot) > 0.0001) {
            ...
            const angleY =
                (normalizedPreviousPosition[1] - normalizedPosition[1]) * rotateIncrementDegrees;
